...
import hashlib
import traceback
from chalicelib.modules.parser import (
    parseMessageText,
    parseExpression,
...
CRITICAL_ERROR_MESSAGE = (
    "Sorry, I'm broken :-( My master will fix me as soon as possible."
)
...
def getInlineResult(inlineResultArticle):
    ...
    inlineResultArticle["id"] = hashlib.md5(
        inlineResultArticle["title"].encode("utf-8")
    ).hexdigest()
...
def command_listAvailableUnits(unitsCategoryName):
    ...
    bitUnits = {
        unitKey: unit for unitKey, unit in categoryUnits.items() if "BIT" in unit["key"]
    }
    helpInfo += formatAvailableUnits(bitUnits)

    byteUnits = {
        unitKey: unit
        for unitKey, unit in categoryUnits.items()
        if "BYTE" in unit["key"]
    }
...
def command_convertInline(expression):
    ...
    if log["type"] == "success":
        response = {"title": log["response"], "message_text": log["fullResponse"]}
    else:
        response = {
            "title": "Error",
            "message_text": BOT_NAME,
...
def convert(expression):
    try:
        ...
        toValueUnit = convertUnit(
            expressionUnits["fromValueUnit"], expressionUnits["toUnit"]
        )
