...
def test_convertUnit():
    ...
    with pytest.raises(Exception) as cm:
        convertUnit({"value": 1.0, "unit": unitsIndex["FOOT"]}, unitsIndex["GRAM"])
        assert (
            str(cm.exception)
    ...
    for fromUnit, toUnit in cases:
        fromValueUnit = {"value": float(fromUnit[0]), "unit": unitsIndex[fromUnit[1]]}

        toValueUnit = {"value": float(toUnit[0]), "unit": unitsIndex[toUnit[1]]}
...
def test_defaultUnits():
    ...
    for categoryName, category in categoriesIndex.items():
        ...
        for unit in category.values():
            ...
            fromValueUnit = {"value": 1.0, "unit": unit}

            result = convertUnit(fromValueUnit, unit["defaultToUnit"])
            errorMessage = (
                f"{unit['key']} and {unit['defaultToUnitKey']} are too different"
            )
