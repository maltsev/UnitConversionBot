...
def test_parseExpression():
    cases = [
        ...
        (
            "19 gram per cubic cm to kilograms per cu metre",
            19,
            "GRAM_PER_CENTIMETER_CUBIC",
            "KILOGRAM_PER_METER_CUBIC",
        ),
        (
            "10 kg per m to grams per centimeters",
            10,
            "KILOGRAM_PER_METER_CUBIC",
            "GRAM_PER_CENTIMETER_CUBIC",
        ),
        ...
        # Pressure
        (
            "1 hPa to pound-force per square inch",
            1,
            "HECTOPASCAL",
    ...
    invalidUnitErrorTemplate = (
        "Sorry, I'm just a stupid bot :-( I don't know what does '{}' mean. "
    ...
    for expression, value, fromUnitName, toUnitName in cases:
        assert parseExpression(expression, unitsIndex) == {
            "fromValueUnit": {"value": value, "unit": unitsIndex[fromUnitName]},
...
def test_normalizeExpression():
    ...
    for expression, expectedNormalizedExpression in cases:
        assert (
            normalizeExpression(expression, unitsIndex) == expectedNormalizedExpression
        )
